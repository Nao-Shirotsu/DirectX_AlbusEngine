...
private:
  ...
  auto CreateDevice(const UINT width, const UINT height) -> HRESULT;
  auto CreateVertexPosBuffer() -> void;
  auto CreateVertexColorBuffer() -> void;
  auto CreateVertexIndexBuffer() -> void;
  auto CreateRenderTargetView() -> HRESULT;
  auto SetViewport(const UINT width, const UINT height) -> void;
  auto DefineInputLayout(ID3DBlob* blobVS) -> HRESULT;
  auto CreateBlendState() -> HRESULT;
  auto CreateRasterizerState() -> HRESULT;
  auto CreateDepthStencil() -> HRESULT;
